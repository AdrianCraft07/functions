function load(config) {
  config ||= {}
  let = { loadingText = 'Loading', loadingAnimation = '|/-\\', loadingSpeed = 100, loadingFormat = '{loadingText} {loadingAnimation}', endLoading = () => { console.log('load finally') }, loadingLog = (...args) => console.log(...args) } = config;
  ...
  let loadingAnimationInterval = setInterval(() => {
    ...
    if (loading) process.stdout.write('\r' + loadingFormat.replace(/{loadingAnimation}/, loadingAnimation[loadingAnimationIndex]));
    else {
